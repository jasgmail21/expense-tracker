...
import { dayLabel, fmtMoney, fmtSigned, monthLabel } from "../utils";

export function FlowBarTransactionsModal({
  monthKey,
  type,
  onClose,
  onEdit,
          ...
          <div className="flex items-center gap-3">
            <span
              className={`grid h-10 w-10 place-items-center rounded-lg text-paper ${
                type === "income" ? "bg-moss" : "bg-coral"
              }`}
            >
              <Icon name={type === "income" ? "upRight" : "downRight"} size={18} />
            </span>
            <div>
              <h2 className="font-display text-lg font-bold text-ink">
                {type === "income" ? "Income" : "Expenses"} · {monthLabel(monthKey)}
              </h2>
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-ink-soft">
                  {type === "income" ? "Income" : "Expense"} transactions for this month will appear here.
                </p>
              </div>
            </div>
          ) : (
              ...
              {txs.map((t) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <Icon name={(cat?.icon as any) ?? "coins"} size={16} />
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold text-ink">{t.note || cat?.name || "Untitled"}</p>
                      <p className="text-[12px] text-ink-faint">
                        {cat?.name ?? "Uncategorized"} · {dayLabel(t.date)}
